 ...
 * Lundi à samedi, 8h-17h, blocs de 4h
 ...
 */
function generer_planning($salles, $promotions, $cours, $options, $creneaux_disponibles, $preference_utilisateur = null) {
    $planning = [];
    $index_cours = 0; // Compteur pour répartir les cours sur les créneaux
    ...
    foreach ($cours_tries as $cours_item) {
        ...
        if ($cours_item['type'] === 'tronc_commun') {
            ...
            if ($preference_utilisateur && isset($preference_utilisateur['promotion']) && $preference_utilisateur['promotion'] === $id_promotion) {
                $effectif = $preference_utilisateur['effectif'];
            }
        } else {
        ...
        // Chercher un créneau libre pour ce groupe en commençant à un index différent pour répartir
        $start = $index_cours % count($creneaux_disponibles);
        $creneau_trouve = false;
        
        for ($j = 0; $j < count($creneaux_disponibles); $j++) {
            $i = ($start + $j) % count($creneaux_disponibles);
            ...
            $salle_trouvee = null;
            if ($preference_utilisateur && isset($preference_utilisateur['promotion']) && $preference_utilisateur['promotion'] === $id_promotion) {
                $salle_preferee = $preference_utilisateur['salle'];
                if (isset($salles[$salle_preferee]) && salle_disponible($planning, $salle_preferee, $creneau) && capacite_suffisante($salles, $salle_preferee, $effectif)) {
                    $salle_trouvee = $salle_preferee;
                }
            }
    ...
    foreach ($options as $id_option => $option) {
        ...
        $cours_option = null;
        foreach ($cours as $c) {
            if ($c['type'] === 'option' && isset($c['promotion']) && $c['promotion'] === $option['promotion_parent']) {
                $cours_option = $c;
                break;
            }
        }
        ...
        // Chercher un créneau libre pour ce groupe en commençant à un index différent
        $start = $index_cours % count($creneaux_disponibles);
...
function rapport_occupation_salles($planning, $salles) {
    $rapport = [];
    $creneaux_total = 12; // 6 jours × 2 créneaux
