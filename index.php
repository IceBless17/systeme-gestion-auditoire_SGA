...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($formType === 'generate_interactive') {
            $selection_interactive['promotion'] = sanitize_input($_POST['promotion'] ?? '');
            $selection_interactive['salle'] = sanitize_input($_POST['salle'] ?? '');
            $selection_interactive['effectif'] = max(0, (int)($_POST['effectif'] ?? 0));

            if ($selection_interactive['effectif'] <= 0) {
                $erreurs[] = 'Veuillez saisir un effectif d\'étudiants valide.';
            }
            if ($selection_interactive['promotion'] === '' || !isset($promotions[$selection_interactive['promotion']])) {
                $erreurs[] = 'Veuillez choisir une promotion valide.';
            }
            if ($selection_interactive['salle'] === '' || !isset($salles[$selection_interactive['salle']])) {
                $erreurs[] = 'Veuillez choisir un auditoire valide.';
            } elseif ($selection_interactive['effectif'] > 0 && isset($salles[$selection_interactive['salle']]) && $selection_interactive['effectif'] > $salles[$selection_interactive['salle']]['capacite']) {
                $erreurs[] = 'L\'auditoire choisi ne peut pas accueillir ce nombre d\'étudiants.';
            }

            if (empty($erreurs)) {
                $preference_utilisateur = [
                    'promotion' => $selection_interactive['promotion'],
                    'salle' => $selection_interactive['salle'],
                    'effectif' => $selection_interactive['effectif']
                ];
            }
            $action = 'generer';
        }
    ...
    switch ($action) {
        case 'generer':
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($formType) && $formType === 'generate_interactive') {
                $creneaux = generer_creneaux();
                $planning = generer_planning($salles, $promotions, $cours, $options, $creneaux, $preference_utilisateur);
                
                if (empty($erreurs) && sauvegarder_planning($planning, DATA_PATH . '/planning.json')) {
    ...
    <div class="container">
        ...
        <main class="main">
            ...
            <?php elseif ($action === 'generer'): ?>
                ...
                <section class="section">
                    <h2>🛠️ Génération interactive du planning</h2>
                    <p>Entrez le nombre d'étudiants et choisissez un auditoire parmi les propositions disponibles.</p>

                    <form method="post" class="interactive-planning-form">
                        <input type="hidden" name="form_type" value="generate_interactive">
                        ...
                        <div class="form-row">
                            ...
                            <select id="promotion" name="promotion" required>
                                ...
                                <?php foreach ($promotions as $id => $promo): ?>
                                    <option value="<?php echo htmlspecialchars($id); ?>" <?php echo $selection_interactive['promotion'] === $id ? 'selected' : ''; ?>>
                        ...
                        <div class="form-row">
                            <label for="effectif">Nombre d'étudiants</label>
                            <input id="effectif" name="effectif" type="number" min="1" value="<?php echo (int)$selection_interactive['effectif']; ?>" required>
                        </div>

                        <div class="form-row">
                            <label for="salle">Auditoire proposé</label>
                            <select id="salle" name="salle" required>
                                <option value="">Sélectionnez un auditoire</option>
                                <?php foreach ($salles as $id => $salle): ?>
                                    <option value="<?php echo htmlspecialchars($id); ?>" data-capacite="<?php echo (int)$salle['capacite']; ?>" <?php echo $selection_interactive['salle'] === $id ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($salle['designation']); ?> - <?php echo $salle['capacite']; ?> places
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>

                        <div id="capacityError" class="error-message" style="display: none;"></div>

                        <div class="form-note" id="sallePropositions">
                            <strong>Propositions :</strong> Choisissez un auditoire adapté au nombre d'étudiants.
                        </div>

                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary">Générer le planning avec ce choix</button>
                        </div>
                    </form>
                </section>
